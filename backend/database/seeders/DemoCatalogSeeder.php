...
class DemoCatalogSeeder extends Seeder
{
    ...
    private function seedLegalWorkflow(array $movies, array $providers): void
    {
        ...
        foreach (array_values($movies) as $index => $movie) {
            ...
            $documentPath = "legal/demo/{$movie->slug}-contract.pdf";
            $documentFile = $this->ensureDemoLegalDocument($documentPath, $movie->title, $license->contract_number);
            ...
            LegalDocument::updateOrCreate(
                ...
                [
                    ...
                    'checksum_sha256' => $documentFile['checksum'],
    ...
    private function seedUploadWorkflow(array $movies, array $providers): void
    {
        ...
        foreach ($uploads as $index => $item) {
            ...
            $masterPath = "uploads/demo/{$movie->slug}/master.mp4";
            $masterFile = $this->ensureDemoUploadFile($masterPath);
            ...
            $upload->files()->updateOrCreate(
                ...
                [
                    ...
                    'mime_type' => 'video/mp4',
                    'file_size_bytes' => $masterFile['size'],
                    'checksum_sha256' => $masterFile['checksum'],
                    'quality' => '1080p',
                    'duration_seconds' => ($movie->runtime_minutes ?: 90) * 60,
    ...
    private function ensureDemoLegalDocument(string $path, string $movieTitle, ?string $contractNumber): array
    {
        $content = $this->demoPdf([
            'ZMovie demo legal document',
            "Movie: {$movieTitle}",
            'Contract: '.($contractNumber ?: 'Demo contract'),
            'This file is generated by the demo seeder.',
        ]);
    ...
    private function ensureDemoUploadFile(string $path): array
    {
        $source = 'demo-videos/sintel-trailer-720p.mp4';

        if (Storage::disk('public')->exists($source)) {
            $content = Storage::disk('public')->get($source);
        } else {
            $content = "ZMovie demo upload placeholder\n";
        }
