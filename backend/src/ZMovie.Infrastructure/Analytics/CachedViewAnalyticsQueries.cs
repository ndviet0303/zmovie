...
public sealed class CachedViewAnalyticsQueries(IViewAnalyticsQueries inner, IMemoryCache cache) : IViewAnalyticsQueries
{
    private static readonly ConcurrentDictionary<string, SemaphoreSlim> Locks = new();

    public Task<long> GetViewCountAsync(TitleId titleId, CancellationToken ct) =>
        inner.GetViewCountAsync(titleId, ct);
    ...
    public async Task<IReadOnlyList<TopViewCount>> GetTopAsync(TopPeriod period, int limit, DateTimeOffset now, CancellationToken ct)
    {
        ...
        try
        {
            ...
            cached = await inner.GetTopAsync(period, limit, now, ct);
