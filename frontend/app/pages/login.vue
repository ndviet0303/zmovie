...
import { passwordAuth } from "~/services/auth.service";

useHead({ title: "Đăng nhập — ZMovie" });
...
async function submitLogin() {
  if (!login.value.trim() || !password.value || loading.value) return;
  loading.value = true;
  error.value = "";
  try {
    const user = await passwordAuth(login.value.trim(), password.value);
    setUser(user);
    ...
    <section class="relative hidden overflow-hidden lg:block">
      ...
      <div class="relative flex h-full flex-col justify-end p-16 xl:p-24">
        ...
        <p class="mt-5 max-w-md text-lg leading-relaxed text-white/65">
          Nơi mỗi bộ phim mở ra một thế giới mới.
        </p>
      </div>
    </section>
        ...
        >
          ...
          <form class="mt-8 space-y-5" @submit.prevent="submitLogin">
            ...
            <div class="flex items-center justify-between gap-3 text-sm">
              <label
                class="flex cursor-pointer items-center gap-2 text-white/55"
              >
                <input
                  v-model="remember"
                  type="checkbox"
          ...
          <div class="my-7 flex items-center gap-3 text-xs text-white/35">
