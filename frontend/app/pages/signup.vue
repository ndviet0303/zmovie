...
async function submitSignup() {
  ...
  if (!acceptedTerms.value) {
    error.value = "Vui lòng đồng ý với Điều khoản và Chính sách của ZMovie.";
    return;
  }
  ...
  try {
    ...
    error.value =
      ...
      "Đăng ký không thành công. Tài khoản hoặc email có thể đã tồn tại.";
  } finally {
    loading.value = false;
  }
}
...
<template>
  ...
  >
    <section class="relative hidden overflow-hidden lg:block">
      ...
      >
        <ArrowLeft class="size-4" /> Về trang chủ
      </NuxtLink>
      <div class="relative flex h-full flex-col justify-end p-16 xl:p-24">
        ...
        <p class="mt-5 max-w-md text-lg leading-relaxed text-white/65">
          Lưu những thế giới điện ảnh bạn yêu thích, ở một nơi.
        </p>
      </div>
    </section>
    ...
    >
      ...
      <div class="w-full max-w-md">
        <NuxtLink to="/" class="mb-10 flex items-center gap-3 lg:hidden">
          <span
            class="inline-flex size-10 items-center justify-center rounded-xl bg-[#ffb59d]/15 text-[#ffb59d]"
            ><Film class="size-5"
          /></span>
          <span class="font-display text-2xl font-semibold text-[#ffb59d]"
            >ZMovie</span
          >
        </NuxtLink>
        <div
        ...
        >
          ...
          <h1
            class="font-display mt-3 text-4xl font-semibold tracking-[-.035em] text-[#e5e2e1]"
          >
            Tạo tài khoản
          </h1>
          <p class="mt-3 leading-relaxed text-white/55">
            Bắt đầu hành trình điện ảnh được tuyển chọn riêng cho bạn.
          </p>

          <form class="mt-7 space-y-4" @submit.prevent="submitSignup">
            <label class="block">
              ...
              >
                <UserRound class="size-4 shrink-0 text-white/35" />
                <input
                  v-model="name"
                  required
                  autocomplete="name"
                  placeholder="Tên của bạn"
                  class="h-11 w-full bg-transparent text-sm text-white outline-none placeholder:text-white/28"
                />
              </span>
            </label>
            ...
            <label class="block">
              ...
              >
                ...
                <button
                  type="button"
                  class="text-white/40 transition hover:text-[#ffb59d]"
                  :aria-label="showPassword ? 'Ẩn mật khẩu' : 'Hiện mật khẩu'"
                  @click="showPassword = !showPassword"
                >
                  <EyeOff v-if="showPassword" class="size-4" /><Eye
                    v-else
                    class="size-4"
                  />
                </button>
            ...
            <label class="block">
              ...
              >
                ...
                <button
                  ...
                  class="text-white/40 transition hover:text-[#ffb59d]"
                  :aria-label="
                    showConfirmation ? 'Ẩn mật khẩu' : 'Hiện mật khẩu'
                  "
                  @click="showConfirmation = !showConfirmation"
                >
                  <EyeOff v-if="showConfirmation" class="size-4" /><Eye
                    v-else
                    class="size-4"
                  />
                </button>
            ...
            <label
              class="flex cursor-pointer items-start gap-2.5 pt-1 text-sm leading-5 text-white/55"
            >
              <input
              ...
              />
              <span
                class="mt-0.5 grid size-4 shrink-0 place-items-center rounded-[5px] border border-white/25 bg-black/20 text-[11px] font-black leading-none text-[#34201b] transition after:content-[''] peer-checked:border-[#ffb59d] peer-checked:bg-[#ffb59d] peer-checked:after:content-['✓'] peer-focus-visible:ring-2 peer-focus-visible:ring-[#ffb59d]/60 peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-[#201f1f]"
              />
              <span>
                Tôi đồng ý với
                <NuxtLink
                  to="/terms"
                  class="font-medium text-[#ffb59d] hover:text-[#d98367]"
                  >Điều khoản</NuxtLink
                >
                và
                <NuxtLink
            ...
            <button
              type="submit"
              class="flex h-12 w-full items-center justify-center rounded-xl bg-[#ffb59d] text-sm font-semibold text-[#34201b] transition hover:bg-[#d98367] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#ffb59d] disabled:opacity-50"
              :disabled="loading"
            >
          ...
          <div class="my-6 flex items-center gap-3 text-xs text-white/35">
            <span class="h-px flex-1 bg-white/10" />hoặc<span
